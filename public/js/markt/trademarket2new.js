var trade = {
    ...
    //鎻愪氦浜ゆ槗
    saveCoinTrade: function (tradeType, flag) { //flag浠ｈ〃鏄惁闇€瑕佸瘑鐮�
        if ($("#login").val() == "false") {
    ...
    //auto....
    antoTurnover: function (price, money, mtcNum, tradeType) {  //peice鍗曚环  //money  鎬荤殑  mtcnum 涓嶇煡閬�
        if (tradeType == 0) {
                ...
                if (result.code == 200) {
                    ...
                    $.each(result.data.entrutsCur, function (key, value) {
                        ...
                        entrutsCurstr +=
                            ...
                            + value.types +'</td>';
                        entrutsCurstr +=
                            '<td>' + util.numFormat(value.price,
                            trade.cnyDigit)
                            + '</td>';
                        entrutsCurstr +=
                            '<td>' + util.numFormat(value.count,
                            trade.coinDigit)
                            + '</td>';
                        entrutsCurstr +=
                            '<td>' + util.numFormat(value.amount,
                            trade.coinDigit)
                            + '</td>';
                        entrutsCurstr +=
                            '<td>' + util.numFormat(value.leftcount,
                            trade.cnyDigit)
                            + '</td>';
                        entrutsCurstr +=
                            '<td>' + util.numFormat(value.ncount,
                            trade.cnyDigit)
                            + '</td>';

                        entrutsCurstr += '<td class="quotes-intrust-item-btn"><a href="javascript:void(0)" class="cancelEntrustBtc" data-fid="'+value.id+'">'+util.getLan("comm.tips.22")+'</a></td>';
                    ...
                    $.each(result.data.entrutsHis, function (key, value) {
                        ...
                        entrutsHisstr += '<td class="quotes-intrust-item-btn"><a href="javascript:void(0);" class="enhis-btn-detial" data-fid="'+value.id+'">'+util.getLan("comm.tips.21")+'</a></td>';
    ...
    setConvertPrice:function(id,value){
        if (parseInt($("#tradeType").val()) === 2) {
            if (value != "" && value != "0") {
                $("#"+id).text(" 鈮� " + util.numFormat(util.accMul(trade.btcPrice, value), 2) + " CNY");
            }
        }
    }
